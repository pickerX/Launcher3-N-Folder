...
public class InfoDropTarget extends ButtonDropTarget {
    ...
    /**
     * ITEM_TYPE_APPLICATION ShortcutInfo support info drop when we are in disable app app mode
     * AppInfo and PendingAddItemInfo support also.
     * @param context
     * @param info
     * @return
     */
    public static boolean supportsDrop(Context context, Object info) {
        boolean supportDrop = info instanceof AppInfo || info instanceof PendingAddItemInfo;

        if(info instanceof ShortcutInfo) {
            ShortcutInfo shortcutInfo = (ShortcutInfo) info;
            if (LauncherAppState.getInstance().getInvariantDeviceProfile()
                    .isDisableAllApps) {
                supportDrop = shortcutInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION;
            }
        }
