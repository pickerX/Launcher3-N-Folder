...
import static com.gwchina.launcher3.LauncherSettings.AppType.ITEM_TYPE_APP;
import static com.gwchina.launcher3.LauncherSettings.AppType.ITEM_TYPE_FOLDER;

// v2.1
public class AppTypeHelper {
    private static final String TAG = AppTypeHelper.class.getSimpleName();

    private static final String QUERY_INTENT_PHONE =
            "#Intent;action=android.intent.action.DIAL;launchFlags=0x10000000;end";
    public static final String QUERY_INTENT_CONTACTS =
            "content://com.android.contacts/contacts#Intent;action=android.intent.action.VIEW;launchFlags=0x10000000;end";
    private static final String QUERY_INTENT_MMS =
            "#Intent;action=android.intent.action.MAIN;type=vnd.android-dir/mms-sms;launchFlags=0x10000000;end";
    private static final String QUERY_INTENT_BROWSER =
    ...
    public synchronized static void configSystemAppIcon(Context context) {
        ...
        AppType[] appTypes =
                {AppType.PHONE, AppType.CONTACTS, AppType.MARKET, AppType.BROWSER, AppType.MMS, AppType.CLOCK, AppType.CALENDAR,
        ...
        for (AppType appType : appTypes) {
            ...
            if (appType == AppType.PHONE) {
                queryIntent = QUERY_INTENT_PHONE;
                isQueryIntent = true;
            } else if (appType == AppType.CONTACTS) {
                queryIntent = QUERY_INTENT_CONTACTS;
                isQueryIntent = true;
            } else if (appType == AppType.MMS) {
                queryIntent = QUERY_INTENT_MMS;
                isQueryIntent = true;
            } else if (appType == AppType.BROWSER) {
                queryIntent = QUERY_INTENT_BROWSER;
                isQueryIntent = true;
            }

            if (isQueryIntent) {
                ActivityInfo info = matchLocalAppsByQueryIntent(context, queryIntent);
                if (info == null) {
                    infos = matchLocalAppsByAppType(context, appType.getValue());
                } else {
                    // lyc 华为手机联系人、电话、短信写在一个程序里面，坑啊。。。
                    if (appType == AppType.PHONE || appType == AppType.MMS) {
                        ActivityInfo contacts = matchLocalAppsByQueryIntent(context, QUERY_INTENT_CONTACTS);
...
//                    Log.e(TAG, "isQueryIntent:直接取第一个就行 " + activityInfo.name);
                } else {
                    // 筛选出唯一的应用，系统应用优先。
                    for (ActivityInfo info : infos) {
                        String packageName = info.packageName;
//                        Log.e(TAG, appType.getValue() + " == " + packageName);
        ...
        for (ResolveInfo resolveInfo : localList) {
            ...
            if (!insertedDefaultWorkspace.contains(packageName)) {
                FolderType folderType = FolderTypeHelper.getFolderTypeByPackage(packageName);
                if (folderType != null) {
//                    Log.e(TAG, packageName + " is " +folderType.getValue());
    ...
    private static void saveToDb(Context context, String appType, ActivityInfo activityInfo,
        ...
        AppTypeDao.save(context, model);
        if (itemType == ITEM_TYPE_APP) {
    ...
    private static void saveToDb(Context context, String appType, ActivityInfo activityInfo,
        ...
        saveToDb(context, appType, activityInfo, insertedDefaultWorkspace, ITEM_TYPE_APP);
    }

    synchronized static List<ActivityInfo> matchLocalAppsByAppType(Context context, String appType) {

        if (sPackageMap.isEmpty()) {
            matchLocalApps(context);
        }
    ...
    synchronized static ActivityInfo matchLocalAppsByQueryIntent(Context context, String queryIntent) {
    ...
    // v2.1 start
    private static void matchLocalApps(Context paramContext) {
        // 查找出本地所有应用
        Intent localIntent = new Intent(Intent.ACTION_MAIN, null);
        localIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.CAMERA.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.MUSIC.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.CLOCK.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.CALENDAR.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.CALCULATOR.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.GALLERY.getValue());

        matchArrayList.clear();
        matchArrayList.add("com.android.settings");
        matchArrayList.add("com.android.settings.MiuiSettings");
        matchLocalApps(paramContext, localList, matchArrayList, AppType.SETTING.getValue());

        matchArrayList.clear();
        matchArrayList.add("com.gwchina.market.activity");
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.MARKET.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.EMAIL.getValue());

        matchArrayList.clear();
        matchArrayList.add("com.google.android.maps.MapsActivity");
        matchArrayList.add("com.baidu.BaiduMap");
        matchLocalApps(paramContext, localList, matchArrayList, AppType.MAPS.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.UPDATER.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.DOWNLOADS.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.PHONE.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.CONTACTS.getValue());

        matchArrayList.clear();
        matchArrayList.add("com.google.android.browser");
        matchArrayList.add("com.android.browser");
        matchArrayList.add("com.android.chrome");
        matchLocalApps(paramContext, localList, matchArrayList, AppType.BROWSER.getValue());
        ...
        matchLocalApps(paramContext, localList, matchArrayList, AppType.MMS.getValue());
        matchArrayList.clear();
    }
     ...
     * @param appType        应用类型
     */
    private static void matchLocalApps(Context paramContext, List<ResolveInfo> localList, List<String> matchArrayList,
                                       String appType) {
        // 用于保存匹配的应用信息。
