...
public abstract class SQLiteCacheHelper {
    ...
    public boolean updateTransactionWithSameClause(List<ContentValues> values, String whereClause, List<String> whereArgs) {
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        try {
            db.beginTransaction();
            for (int i = 0; i < values.size(); i++) {
                db.update(mTableName, values.get(i), whereClause, new String[]{whereArgs.get(i)});
            }
            db.endTransaction();
            return true;
        } catch (SQLiteFullException e) {
